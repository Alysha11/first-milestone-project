...
## Band site for The Monkees

This project is a website for fans (existing and new) of 1960’s rock band ‘The Monkees’.  It a responsive and mobile-first designed performing well on different screen sizes. This website is static apart from the navigation bar (created through Bootstrap 4).

It simple to use with easy navigation and is aesthetically appealing with some nostalgic elements to represent the bands history.  Together with the ease of use and look the site is a place the band can use to showcase their music, publicise availability to perform at events and highlight a social media presence thus fully fulfilling the brief that was provided. 
...
[Desktop](https://github.com/Alysha11/first-milestone-project/blob/master/other/desktopWireframes.pdf)

 

[Mobile](https://github.com/Alysha11/first-milestone-project/blob/master/other/mobileWireframes.pdf)
...
The navigation is a simple hamburger style dropdown on the top right of mobile view. When open the page headings are displayed with the branding on top. Clicking on the brand will take users to the home page however there is also a ‘home’ page listed which is more explicit.
...
The page welcomes users to the site. It features a banner with some text which is a feature consistent throughout the site. It provides clear information for users on where they are and what they can expect.

There is a psychedelic background over which a cartoon representation of the band is featured. The background was created by using CSS gradients and is reminiscent of 1960s styling. It is a bold background and draws in the attention of the user and all text is on more muted backgrounds so that it is still friendly for those reading text.
...
At the bottom of all the pages there is a red flower image that was used as another nod to the era of the bands beginnings. It forms an attractive and contrasting separation between the page content and the social links footer.
...
_Something that was highlighted was that I had put the flower divider icon in a section tag which does not contain a h1. To rectify this I read up on [semantic elements](https://www.w3schools.com/html/html5_semantic_elements.asp) and felt that the figure tag most closely fit that part of the code and replaced the section tag with figure tag and as a result had no errors using the validators._

 

2. I tested that the navigation work as expected using a number of browsers and screen sizes.

 

3. Checked that other links direct correctly (social, youTube videos and news articles).

 

4. Checked that the layout was as expected in the different browsers and at different screen sizes.

 
_One difficulty I came across was with the audio players. On a smaller desktop screen size the individual players would touch each other. I was able to fix this by adding media queries to reduce, then increase the audio player width according to screen size_

 
5.Tested the audio players worked by playing them.
 

6.Tested the videos worked by playing them.
...
- Zoom effect on article images were created with help from [CSS Tricks ]( https://css-tricks.com/zooming-background-images/)

- Inspiration for the psychedelic gradient and guidance on how to create came from [W3Schools](https://www.w3schools.com/css/css3_gradients.asp).
